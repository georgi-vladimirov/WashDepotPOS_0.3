...
def save_expence(*, expence: Expence) -> Tuple[bool, str]:
    transaction = create_tran_for_expence(date = expence.date, amount = expence.amount, details = expence.name)
    transaction.save()
...
def delete_expence(*, expence: Expence) -> Tuple[bool, str]:
    result = transaction_delete(transaction=expence.transaction) #When transaction is deleted, expence is also deleted due to CASCADE
    if result:

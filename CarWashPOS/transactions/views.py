...
from .selectors import (
    get_trans_by_cal_event,
    get_cash_end_from_prev_cal_event,
    get_tran_by_id,
)
from .services import (
    daily_report_calculate,
    transaction_save,
    transaction_operation_save,
...
class TransactionsOverview(LoginRequiredMixin, View):
    def get(self, request):
        ...
        aggregates = daily_report_calculate(
            transactions_qs=transactions, filters=FILTERS
        )
...
class Transactions(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        tran_type = (
            TranType(tran_type_str) if tran_type_str in TranType.values else None
        )
        ...
        if tran_type == TranType.START:
            ...
            amount: Decimal = (
                last_end_trans.amount if last_end_trans else Decimal("0.00")
            )
